...
def scale_to_h(img, target_height):
    # From Calamari's scale_to_height_processor
    h, w = img.shape
    f = target_height / h
    if f == 1:
        return img
...
def center_normalize(img, line_height=48, extra_params=(4, 1.0, 0.3)):
    # Adapted from Calamari's CenterNormalizer
    range_val, smoothness, extra = extra_params
    target_height = line_height

    # Scale to target height first (CenterNormalizer does this internally?)
...
def final_prep(img, normalize=True, invert=True, transpose=True, pad=16, pad_value=0):
    ...
    if pad > 0:
        if transpose:
            w = data.shape[1]
            data = np.vstack([
                np.full((pad, w, channels), pad_value),
                data,
                np.full((pad, w, channels), pad_value),
            ])
        else:
            w = data.shape[0]
            data = np.hstack([
                np.full((w, pad, channels), pad_value),
                data,
                np.full((w, pad, channels), pad_value),
            ])
...
# Calamari's preprocessing pipeline
img_center_normalized = center_normalize(img_np, line_height=48)
img_final = final_prep(img_center_normalized)

# Now, add back the batch and channel dimensions (FinalPreparation squeezes it)
img_final = np.expand_dims(img_final, axis=0)  # Batch
img_final = np.expand_dims(img_final, axis=-1)  # Channel
...
logits = result['root'][0].numpy()  # (seq_len, charset_size)
chars = tf.argmax(logits, axis=-1).numpy()  # (seq_len,)

# CTC decode: merge consecutive duplicates and remove blanks
decoded_norepeat = []
prev = -1
for c in chars:
    if c != prev:
        if 0 < c < len(charset):  # 0 is blank
            decoded_norepeat.append(charset[c])
    prev = c

result_text = ''.join(decoded_norepeat)
print('Decoded result:', repr(result_text))
