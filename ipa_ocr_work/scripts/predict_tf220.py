...
print(f"First 50 chars: {charset[:50]}")
...
with open('e:/my_pro/result_tf220.txt', 'w', encoding='utf-8') as f:
    f.write(result_text)

with open('e:/my_pro/char_freq_tf220.txt', 'w', encoding='utf-8') as f:
    f.write('Top 20 chars:\n')
    for idx, count in char_freq[:20]:
        if idx < len(charset):
            f.write(f'{repr(charset[idx])} (index {idx}): {count}\n')

print('Saved result to e:/my_pro/result_tf220.txt')
print(f'Top 10 chars:')
for idx, count in char_freq[:10]:
    if idx < len(charset):
