...
def prep_with_binary(img, invert=True, transpose=True, pad=16, pad_value=0):
    # First, threshold to binary
    _, binary = cv.threshold(img, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)
    data = binary.astype(np.float32) / 255.0
    if len(data.shape) != 3:
...
img_final = prep_with_binary(img_scaled)
...
with open('e:/my_pro/binary_result.txt', 'w', encoding='utf-8') as f:
    f.write(result_text)

print('Saved result to e:/my_pro/binary_result.txt')
