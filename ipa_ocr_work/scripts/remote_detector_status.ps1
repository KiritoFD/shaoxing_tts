...
$PidPath = Join-Path $RunRoot "tone_detector_remote_pid.txt"
...
$History = Join-Path $RunRoot "tone_detector_remote_b512_e100\history.tsv"
if (Test-Path $History) {
    Get-Content -LiteralPath $History -Tail 10
}

Write-Host "CHECKPOINT_EVAL"
$Eval = Join-Path $RunRoot "tone_detector_remote_b512_e100\checkpoint_eval.tsv"
if (Test-Path $Eval) {
...
Get-CimInstance Win32_Process |
    Where-Object { $_.CommandLine -like "*train_tone_position_detector*" -or $_.CommandLine -like "*run_tone_detector_remote*" } |
