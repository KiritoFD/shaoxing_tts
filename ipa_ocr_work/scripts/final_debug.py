...
preprocessed_img.save('e:/my_pro/preprocessed_debug.png')
...
logits = result['root'][0, :, :].numpy()  # (seq_len, 388)
chars = tf.argmax(logits, axis=-1).numpy()  # (seq_len,)

# Save logits for analysis
np.save('e:/my_pro/logits_debug.npy', logits)
np.save('e:/my_pro/chars_debug.npy', chars)
...
with open('e:/my_pro/char_freq_debug.txt', 'w', encoding='utf-8') as f:
    f.write('Top 20 most frequent characters:\n')
    for idx, count in char_freq[:20]:
        if idx < len(charset):
            f.write(f'{repr(charset[idx])} (index {idx}): {count}\n')
...
with open('e:/my_pro/final_result_debug.txt', 'w', encoding='utf-8') as f:
    f.write(result_text)

print('Saved all debug files')
